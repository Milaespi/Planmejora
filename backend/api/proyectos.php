...
try {
    switch ($method) {
        ...
        case 'GET':
            if ($id) {
                // Obtener proyecto con sus fases y actividades
...
function validarProyecto(array $body): void {
    $requeridos = ['nombre', 'direccion', 'cliente', 'fecha_inicio', 'fecha_fin_estimada'];
    foreach ($requeridos as $campo) {
        if (empty($body[$campo])) jsonError("El campo '$campo' es requerido");
    }
    if (strlen(trim($body['nombre'])) < 5)     jsonError('El nombre debe tener al menos 5 caracteres');
    if (strlen(trim($body['direccion'])) < 10)  jsonError('La dirección debe tener al menos 10 caracteres');
...
function crearFasesYActividades(int $proyectoId, string $fechaInicio): void {
    $actividadesF1 = [
    ...
    foreach ([
        ['Obra Blanca', 'obra_blanca', 1, $actividadesF1],
        ['Amueblamiento', 'amueblamiento', 2, $actividadesF2],
    ] as [$nombreFase, $tipo, $orden, $actividades]) {
